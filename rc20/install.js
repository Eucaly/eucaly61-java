...
function previewEasy(which){
	...
	case 2: //preset parameters
		...
		var commentFormat =	{
			idefault: '%rcAuthorLinkFormat% 於 %rcTitleLinkFormat% %replyImg% %rcSay% &#12300;%content%&#12301; &nbsp;- %timestamp%',
			wretch:   '%rcTitleLinkFormat%%replyImg% %rcSay% &#12300;%content%&#12301; &nbsp;by %rcAuthorLinkFormat% (%timestamp%)'
			};
		switch(preSetting){
		case 0:
		...
		rcPreSetting.showJumpButton= document.makeWidget.useJump.checked;
		disableInput('jumpMsgT',true);document.makeWidget.jumpMsgT.value = '跳至留言';
		disableInput('jumpMsgB',true);document.makeWidget.jumpMsgB.value = '我跳';
		uncheckBox('shownoTitle',true);rcPreSetting.showRCnoPost= document.makeWidget.shownoTitle.checked;
		disableInput('noTitleMsg',false);document.makeWidget.noTitleMsg.value = '原文已被刪除';
		document.makeWidget.nothing.value = '<p>沒有留言可以顯示</p>';
		document.makeWidget.openAllMsg.value = '全部展開';
		document.makeWidget.hideAllMsg.value = '全部隱藏';		
		document.makeWidget.openMsg.value = '留了言';
		document.makeWidget.openImg.value = 'http://lvchen.atspace.com/RC20/rc_0609_f.gif';
		document.makeWidget.closeMsg.value = '留言說:';
		document.makeWidget.closeImg.value = 'http://lvchen.atspace.com/RC20/rc_0609_uf.gif';
		document.makeWidget.loadingMsg.value = '<img src="http://lvchen.atspace.com/RC20/2-0.gif"/>&nbsp;載入中...';
		document.makeWidget.goPostImg.value = 'http://lvchen716.googlepages.com/external.png';
		document.makeWidget.goPostMsg.value = '直接去留言';
		document.makeWidget.prevPage.value = '上一頁';
		document.makeWidget.nextPage.value = '下一頁';				
		document.makeWidget.footerMsg.value = '您正在看留言 %range%，共有 %totalNum% 則留言';
...
function saveValue()	{
	...
	settingString +="\<script type='text/javascript' src='http://lvchen-recentcomments.googlecode.com/svn/trunk/RecentComment/RC20/rc20_pack.js'\>\<\/script\>\n";
